...
#endif
#if GPR_TOO
    register unsigned x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12,
        x13, x14, x15;
    x0 = chacha_const[0];
    x1 = chacha_const[1];
    x2 = chacha_const[2];
...
static inline void print_init(const uint64_t* const restrict in,
                              ...
                              const uint8_t num) {
  printf("static const uint64_t %s%u[%zu] = { ", name, num, inlen);
  printf("%#018llx", in[0]);
...
int main(void) {
  ...
  posix_memalign((void**)&key, 32, 32);
  posix_memalign((void**)&nonce, 16, 8);
  posix_memalign((void**)&keystream, 16, TESTLEN);

  // Set the initial key and nonce to zeros.
  memset(key, 0, 32);
  memset(nonce, 0, 8);

  for (size_t i = 0; i < 256; i++) {
    // Clear the keystream buffer each iteration.
    memset(keystream, 0, TESTLEN);
    crypto_stream_xor((uint8_t*)keystream,
                      (uint8_t*)keystream,
                      TESTLEN,
                      (uint8_t*)nonce,
                      (uint8_t*)key);
    print_init(keystream, TESTLEN / 8, "keystream", i);
    print_init(key, 4, "key", i);
    print_init(nonce, 1, "nonce", i);
    printf("\n");
    memcpy(key, ((uint8_t*)keystream) + i, 32);
    memcpy(nonce, ((uint8_t*)keystream) + 32 + i, 8);
  }
}
